...
def main(argv):
    ...
    # now save trajectories into zarr files
        ...
        for j , trajectory in tqdm(enumerate(all_trajectories), total=len(all_trajectories), desc="Processing trajectories"):
            trajectory_group = root.create_group(str(group))
            group += 1
            ...
            observations = {field: [] for field in obs_fields}
            infos = {field: [] for field in info_fields}
            actions = {field: [] for field in action_fields}

            for entry in trajectory:
                for field in obs_fields:
                    observations[field].append(entry[field])
                for field in info_fields:
                    infos[field].append(entry[field])
                for field in action_fields:
                    actions[field].append(entry[field])

            for field, values in observations.items():
                obs_group.create_dataset(field, data=np.array(values), chunks=(400,))

            for field, values in infos.items():
                info_group.create_dataset(field, data=np.array(values), chunks=(400,))

            for field, values in actions.items():
                action_group.create_dataset(field, data=np.array(values), chunks=(400,))
            # break
        print("Wrote dataset")
